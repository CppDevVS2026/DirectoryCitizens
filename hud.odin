...
event_color :: proc(kind: EventKind) -> rl.Color {
	switch kind {
...
draw_hud :: proc(s: ^GameState) {
	...
	// Mouse wheel scrolling (only when mouse is over the panel)
	...
	for idx := s.citizen_scroll; idx < s.citizen_scroll + vis_count && idx < i32(len(s.citizens)); idx += 1 {
		...
		bg := COL_SEL if isel else rl.Color{0, 0, 0, 0}
		rl.DrawRectangle(px + 3, iy + 1, pw - 6, item_h - 2, bg)
	...
	if i32(len(s.citizens)) > vis_count {
		dot_x := px + pw / 2 - i32(len(s.citizens)) * 5
		dot_y := y + list_h + 4
		for i in 0..<len(s.citizens) {
			col := COL_ACCENT if i32(i) == s.citizen_scroll else COL_DIVIDER
			rl.DrawCircle(dot_x + i32(i) * 10, dot_y, 3, col)
		}
	}
...
draw_stat_bar :: proc(x, y, w: i32, label: cstring, value: f32, bar_col: rl.Color) {
	...
	fill := i32(f32(bw) * value / 100)
	rl.DrawRectangle(bx, y, fill, bar_h, bar_col)
